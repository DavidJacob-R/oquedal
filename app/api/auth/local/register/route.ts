...
export async function POST(req: Request) {
  ...
  try {
    ...
    // Evitar emails repetidos en ambas tablas
    ...
    if (existsAuth.rowCount) {
      ...
      return NextResponse.json({ ok: false, msg: "el correo ya está registrado" }, { status: 409 });
    }
    ...
    const newId = insUser.rows[0].id as string;
    const displayName = (insUser.rows[0].nombre as string) || "Cliente";
    ...
    const res = NextResponse.json({
      ok: true,
      user: { id: newId, nombre: displayName, rol: "cliente" as const },
      redirect: "/cliente/pedidos",
    });
    // Set cookie de cliente y limpiar admin
