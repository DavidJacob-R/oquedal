  ...
  try {
    await tx(async (client) => {
      const r = await client.query(
        `SELECT pr.id, r.conductor_id, cd.usuario_id
      ...
      if (String(r.rows[0].usuario_id) !== repUserId) throw new Error("Forbidden");

      await client.query(
        `UPDATE public.parada_ruta SET llegada_real = COALESCE(llegada_real, now()), estado = COALESCE(estado, 'en_proceso') WHERE id = $1`,
        [paradaId]
      );
    });
